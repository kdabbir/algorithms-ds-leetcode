...
// to the first city. In other words, the last distance in the distances  array is equal to the distance from the last city to
...
class Program {
  ...
  public int validStartingCity(int[] distances, int[] fuel, int mpg) {
    int leastCity = 0, leastFuelLeft = 0, currFuelLeft = 0;
		for(int currCity = 1; currCity < distances.length; currCity++) {
			currFuelLeft += mpg * fuel[currCity - 1];
			currFuelLeft -= distances[currCity - 1];
			if(currFuelLeft < leastFuelLeft) {
				leastFuelLeft = currFuelLeft;
				leastCity = currCity;
			}
		}
    return leastCity;
