...
class Solution {
    ...
    int[] topologies = new int[n + 1];
		topologies[0] = 1;
		topologies[1] = 1;
		for(int topologyIdx = 2; topologyIdx <= n; topologyIdx++) {
			for(int rootIdx = 1; rootIdx <= topologyIdx; rootIdx++) {
				topologies[topologyIdx] += topologies[rootIdx - 1] * topologies[topologyIdx - rootIdx];
			}
		}
